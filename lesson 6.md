...
- Pod может состоять минимум из 2х контейнеров
    ...
    - второй контейнер - так называемый служебный контейнер, он держит в себе сетевой неймспейс, [pause-контейнер](https://stackoverflow.com/questions/48651269/what-are-the-pause-containers), его мы особо не касаемся на практике
...
- Рассматриваем настройки probes
    - readinessProbe
        ...
        - httpGet
            ...
            - успешными считаются коды ответа в диапазон от 200 до 399
...
- Если сходить в логи пода, можно увидеть информацию о запросах probes к nginx
...
### Service
- Ключевые моментов почти для всех сервисов
...
- NodePort
    ...
    - Может публиковать приложение только на определённом диапазоне портов
    - Создаёт на нодах соответствующее правило трансляции
