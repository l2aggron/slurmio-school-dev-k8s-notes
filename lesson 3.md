...
- Выдача ресурсов настраивается двумя параметрами (добавить ссылку на слайд):
    - Limits
...
- Бывают 3х видов:
    - Best Effort - когда мы не ставим на наше приложение никаких лимитов и реквестов, оно может приехать на любую ноду кластера и использовать все доступные ресурсы, но если ресурсов не будет хватать, то приложение сможет получить только те ресурсы, которые свободны
    - Burstable - когда у нас указаны реквесты, но не указаны лимиты, либо лимиты больше чем реквесты, наш файл описания деплоймента выше это демонстрирует, то есть, мы запрашиваем у ноды 10 millicpu, но иногда мы можем начать потреблять до 100 millicpu
    - Guaranteed
- От типа QoS зависит ещё то, как kubernetes будет эвакуировать приложения
    - Например, если на ноде началась нехватка ресурсов и там есть поды с QoS классом Best Effort, то это поды будут удалены с ноды в первую очередь, чтобы освободить ресурсы для оставшихся подов с остальными QoS классами, чтобы они могли продолжать работать

# Остановился тут:
https://youtu.be/LLVfC08UVqY?list=PL8D2P0ruohOBSA_CDqJLflJ8FLJNe26K-&t=5464
